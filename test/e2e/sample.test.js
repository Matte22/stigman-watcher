...
import { writeToHistoryFile, createAsset, runWatcher, uploadTestStig, clearDirectory, createCkl, waitFor,clearHistoryFileContents, initWatcherTestCollection, startApi, startAuth, startDb, stopProcesses, initNetwork, runWatcherPromise } from "./lib.js"
...
// 		const apiHost = api.getHost()
// 		const apiPort = api.getMappedPort(54000)
// 		env.apiBase = `http://${apiHost}:${apiPort}/api`
...
// 		await createCkl(BASE_CKL_PATH, `${nestedBase}/nested1.ckl`, 'nested1')
// 		await createCkl(BASE_CKL_PATH, `${nestedBase}/nested2.ckl`, 'nested2')
// 		await createCkl(BASE_CKL_PATH, `${nestedBase}/nested3.ckl`, 'nested3')
...
//     await waitFor(() => watcher.logRecords.some(r => r.message === 'running'), 100000)
//     expect(watcher.logRecords.some(r => r.message === 'running')).to.be.true
//   })
...
//     const originalContents = fs.readFileSync(`${dropEnv.path}/dropped.ckl`, 'utf8')
...
//     await delay(5000)
//     fs.writeFileSync(`${dropEnv.path}/dropped2.ckl`, modifiedContents, 'utf8')
...
//     await waitFor(() => watcher.logRecords.some(r => r.message === 'watching'), 20000)
...
//     await waitFor(() => watcher.logRecords.some(r => r.message === 'file system event'), 20000)
...
//     await waitFor(() => watcher.logRecords.some(r => r.component === 'events' && r.message === 'file system event' && r.file.endsWith('dropped.ckl' ), 70000))
...
//     expect(log.file).to.equal(`${dropEnv.path}/dropped.ckl`)
//   })

//   it('Should start a batch of size 1', async () => {
//     await waitFor(() => watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'batch started' && r.size === 2), 20000)
...
//     await waitFor(() => watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'asset data received'), 20000)
//     expect(watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'asset data received')).to.be.true
//     await waitFor(() => watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'stig data received'), 20000)
//     expect(watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'stig data received')).to.be.true
//   })

//   it('processes the dropped file and adds to history', async () => {
//     await waitFor(() => watcher.logRecords.some(r => r.component === 'cargo' && r.message === 'batch ended'), 20000)
...
//     try {
//       fs.unlinkSync(`${dropEnv.path}/dropped.ckl`)
//     } catch(e) {}
//   })
// })
...
//     env.collectionId = collection.collectionId
//     await uploadTestStig('VPN_STIG.xml')
//     watcher = await runWatcherPromise({ entry: "index.js", env, consoleLog: true, resolveOnMessage: `finished one shot mode - no work to do`})
//   })
...
//   before(async () => {
//     await writeToHistoryFile(env.historyFile, ['test/e2e/testFiles/test.ckl'])
//     await initNetwork()
//     db = await startDb()
//     auth = await startAuth()
//     env.authority = `http://localhost:${auth.port}`
...
//     env.apiBase = apiBase
//     const { user, collection } = await initWatcherTestCollection()
//     watcher = await runWatcherPromise({ entry: "index.js", env: env, consoleLog: true, resolveOnMessage: 'running' })
//     env.collectionId = collection.collectionId
//     await uploadTestStig('VPN_STIG.xml')
//     watcher = await runWatcherPromise({ entry: "index.js", env, consoleLog: true, resolveOnMessage: `received shutdown event with code 0, exiting`})
//   })
...
//     }
//     for (let i = 1; i <= 5; i++) {
//       await createCkl(BASE_CKL_PATH, `${ignorePath}/ignore${i}.ckl`, `ignore${i}`)
//     }
...
//     await createAsset(null, collection.collectionId)
// 		await uploadTestStig('VPN_STIG.xml')
// 		watcher = await runWatcherPromise({ entry: 'index.js', env, consoleLog: true, resolveOnMessage: `received shutdown event with code 0, exiting` })
// 	})
...
// describe("Event Mode, Dont Start auth service and go auth offline ", async function () {
//   this.timeout(120_000)
//   let db, auth, api
//   let watcher
//   const dropEnv = {
...
//   it("stops the auth service", async () => {
//     // stop the mock OIDC server so the watcher will see auth failures
//     auth.stop()
//     // which will exercise getToken() and trigger the authOffline alarm/retry logic
//     try {
//       await createCkl(BASE_CKL_PATH, `${dropEnv.path}/trigger-auth-failure.ckl`, 'trigger-auth-failure')
//       // put it in the scrapFiles folder to trigger the watcher
//       await waitFor(() => watcher.logRecords.some(r => r.component === 'scan' && r.message === 'file system event' && r.file && r.file.endsWith('trigger-auth-failure.ckl')), 20000)
//     } catch (e) {}
//   })

//   it("starts and reports config", async () => {
//   // match the shutdown message more robustly (comma/spacing may vary)
//   await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && /shutdown event with code 2/i.test(r.message)), 20333000)
//     // expect(watcher.logRecords.some(r => r.message === 'running')).to.be.true
//     // const running = watcher.logRecords.find(r => r.message === 'running')
//     // expect(running).to.exist
//     // expect(running.options).to.include({ path: dropEnv.path, oneShot: dropEnv.oneShot, cargoSize: dropEnv.cargoSize })
...
describe("Event Mode, stop api and go api offline, come back up, then take api back down and eventually exit", async function () {
  ...
  it("waits for api to come back online", async () => {
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm cleared: apiOffline'), 2002200)
    expect(watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm cleared: apiOffline')).to.be.true
  })
