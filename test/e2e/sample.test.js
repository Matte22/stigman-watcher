...
describe("Scan Mode, Drop in a file while running, then drop in another file for the same asset with new result to update review.", async function () {
  ...
  before(async () => {
    ...
    clearDirectory(env.path)
    await uploadTestStig('VPN_STIG.xml')
    watcher = await runWatcher({ entry: 'index.js', env: env, consoleLog: true })
  })
...
describe("Event Mode, start normal, take down auth service, go offline, bring auth back up and continue.", async function () {
  ...
  before(async () => {
    await clearHistoryFileContents(env.historyFile)
    await clearDirectory(env.path)
    await initNetwork()
    db = await startDb()
    auth = await startAuth()
    env.authority = `http://localhost:${auth.port}`
    api = await startApi()
    env.apiBase = `http://localhost:54001/api` 
    const { collection } = await initWatcherTestCollection()
    env.collectionId = collection.collectionId
    await uploadTestStig('VPN_STIG.xml')
...
describe("Scan Mode, start normal, take down auth service, go offline, bring auth back up and continue.", async function () {
  ...
  it("should start up normally then stop the api service and raise alarm auth offline", async () => {
    ...
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm raised: authOffline'), 100000)
    expect(watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm raised: authOffline')).to.be.true
  })
...
describe("Event Mode, stop api and go api offline, come back up, then take api back down and eventually exit", async function () {
  ...
  it("should start up normally then stop the api service and raise alarm api offline", async () => {
    ...
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm raised: apiOffline'), 20000)
  ...
  it("waits for api to come back online", async () => {
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm lowered: apiOffline'), 20000)
...
describe("Scan Mode, stop api and go api offline, come back up, then take api back down and eventually exit", async function () {
  ...
  it("waits for api to come back online", async () => {
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm lowered: apiOffline'), 2002200)
    expect(watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm lowered: apiOffline')).to.be.true
  })
...
describe("Start watcher without an API", function () {
  ...
  before(async () => {
    ...
    env.authority = `http://localhost:${auth.port}`
    env.apiBase = `http://localhost:54001/api` 
    watcher = await runWatcherPromise({ entry: 'index.js', env: env, consoleLog: true })
  })

  after(async () => {
    stopProcessesOnly([auth, db])
  })
...
describe("Start watcher without an auth service", function () {
  ...
  before(async () => {
    ...
    env.authority = `http://localhost:${auth.port}`
    api = await startApi()
    env.apiBase = `http://${api.getHost()}:${api.getMappedPort(54000)}/api`
    const { collection } = await initWatcherTestCollection()
    env.collectionId = collection.collectionId
    await uploadTestStig('VPN_STIG.xml')
    await auth.stop()
...
describe("Start watcher against a non existent collection (stimulates noGrant alarm)", function () {
  ...
  it("should start up and try to connect to the api and raise alarm noGrant", async () => {
    // ensure watcher finished start/preflight and is running
    await waitFor(() => watcher.logRecords.some(r => r.message === 'running'), 20000)
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm raised: noGrant'), 20000)
...
describe("Should raise noToken alarm when auth is reachable but no token is provided", function () {
  ...
  before(async () => {
    ...
    env.authority = `http://localhost:${auth.port}`
    api = await startApi()
    const { collection } = await initWatcherTestCollection() // create a collection but do not give the watcher access to it
    env.collectionId = collection.collectionId
    env.apiBase = `http://${api.getHost()}:${api.getMappedPort(54000)}/api`
    await uploadTestStig('VPN_STIG.xml')
    watcher = await runWatcher({ entry: 'index.js', env: env, consoleLog: true })
  ...
  it("should start up and try to connect to the api and raise alarm noToken", async () => {
    // ensure watcher finished start/preflight and is running
    await waitFor(() => watcher.logRecords.some(r => r.message === 'running'), 20000)
    await waitFor(() => watcher.logRecords.some(r => r.component === 'index' && r.message === 'Alarm raised: noToken'), 20000)
