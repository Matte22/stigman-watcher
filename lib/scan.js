...
async function startScanner (context) {
  try {
    ...
    let { history, historyStream, lineCount, config } = context;
    ...
    if (config.historyFile && history.size !== lineCount) {
      ...
      ({ history, historyStream, lineCount, config } = context); // Destructure updated context 
    }
...
async function _writeHistory(context, entry, rewrite = false) {
  try {
    ...
    if (rewrite) {
      ...
      const updatedContext = {...context, historyStream: newHistoryStream, lineCount: history.size};
