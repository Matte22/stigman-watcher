...
async function _writeHistory(entry, rewrite = false) {
  try {
    if (rewrite) {
      ...
      // close append stream
      if (historyStream) {
        await new Promise(resolve => historyStream.end(resolve));
      }
      ...
      // close
      await new Promise((resolve) => rewriteStream.end(resolve));

      // Reopen the aoppend historyStream
      historyStream = fs.createWriteStream(config.historyFile, { flags: 'a' });
    } else {
      
      if (!historyStream || historyStream.writableEnded) {
        historyStream = fs.createWriteStream(config.historyFile, { flags: 'a' });
      }
      // Write single file entry
