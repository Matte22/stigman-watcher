...
let writeQueue = new Queue((batch, done) => {
  // Join the batch entries with a newline character between each
  const data = batch.join('\n') + '\n';
  fs.writeFile(historyFilePath, data, (err) => {
      if (err) {
          logger.error({
              component: 'historyManager',
              error: serializeError(err),
              message: 'Failed to write history to file'
          });
      } else {
          logger.info({
              component: 'historyManager',
              message: 'History batch written to file',
              file: historyFilePath
          });
      }
      done();
  ...
  batchSize: 5,
  batchDelay: 20000, // 20 seconds
  batchDelayTimeout: 100000, // 5 minutes
});
...
function addToHistory(entry) {
  ...
  history.add(entry);
  writeQueue.push(Array.from(history));
